...
class PanaromaStitcher:
    ...
    def image_stitch(self, images, lowe_ratio=0.75, max_Threshold=5, match_status=False):
        ...
        key_ptsA, key_ptsB, matches = self.match_keypoints(key_points_A, key_points_B, features_of_A, features_of_B, lowe_ratio)
        (Homography, status) = self.compute_homography(key_ptsA, key_ptsB, max_Threshold)
        if (matches, Homography, status) is None:
            return None

        manual_Homography_matrix = self.homography_ransac(key_ptsA, key_ptsB, max_Threshold)
        self.homography_matrices.append(manual_Homography_matrix)

        result_image = self.get_warp_perspective(imageA, imageB, manual_Homography_matrix)
    ...
    def warp_perspective_manual(self, imageA, imageB, Homography):
